...
public class PDF extends FunctionBase2 {
     ...
     * For d=1, accepts a single number.
     * For d>1, would need to accept an array (future enhancement).
     */
    private double[] extractPoint(NodeValue node, int d) {
        if (d != 1) {
            throw new UnsupportedOperationException(
                "PDF evaluation currently only supports 1-dimensional GMMs. " +
                "Got d=" + d);
        }
        
        if (!node.isNumber()) {
            throw new IllegalArgumentException(
                "Second argument must be a number for 1D GMM evaluation");
        }
