...
public class AlgebraGenerator
{
    ...
    protected Op compileElement(Element elt) {
        ...
        // ProbSPARQL Extensions
        if ( elt instanceof ElementFuseJoin elt2 )
            return compileElementFuseJoin(OpTable.unit(), elt2);
        if ( elt instanceof ElementSimilarityJoin elt2 )
            return compileElementSimilarityJoin(OpTable.unit(), elt2);
    ...
    // ========================================================================
    // ProbSPARQL Extensions: FUSEJOIN and SIMILARITYJOIN
    // ========================================================================

    /**
     * Compile ElementFuseJoin into OpFuseJoin.
     * 
     * FUSEJOIN performs Bayesian fusion of probability distributions when 
     * JS divergence between two GMM variables is within tolerance.
     * 
     * Supports both relational semantics (dual patterns) and legacy semantics (single pattern).
     */
    protected Op compileElementFuseJoin(Op current, ElementFuseJoin fuseJoin) {
        ...
        if (fuseJoin.hasLeftPattern()) {
            // Relational semantics: { leftPattern } FUSEJOIN(...) { rightPattern }
            Op leftOp = compileElement(fuseJoin.getLeftPattern());
            Op rightOp = compileElement(fuseJoin.getRightPattern());
            opFuseJoin = new OpFuseJoin(leftOp, rightOp, leftVar, rightVar, tolerance, resultVar);
        } else {
            // Legacy semantics: FUSEJOIN(...) { pattern }
            Op patternOp = compileElement(fuseJoin.getRightPattern());
            opFuseJoin = new OpFuseJoin(patternOp, leftVar, rightVar, tolerance, resultVar, true);
        }
        
        return join(current, opFuseJoin);
    }

    /**
     * Compile ElementSimilarityJoin into OpSimilarityJoin.
     * 
     * SIMILARITYJOIN filters results based on JS divergence between 
     * two GMM variables being within tolerance.
     * 
     * Supports both relational semantics (dual patterns) and legacy semantics (single pattern).
     */
    protected Op compileElementSimilarityJoin(Op current, ElementSimilarityJoin simJoin) {
