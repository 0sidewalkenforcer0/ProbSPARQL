...
public class TransformCopy implements Transform
{
    ...
    @Override
    public Op transform(OpSemiJoin opSemiJoin, Op left, Op right)   { return xform(opSemiJoin, left, right) ; }
    @Override
    public Op transform(OpAntiJoin opAntiJoin, Op left, Op right)   { return xform(opAntiJoin, left, right) ; }
    @Override
    public Op transform(OpConditional opCond, Op left, Op right)    { return xform(opCond, left, right) ; }

    // ProbSPARQL Extensions
    @Override
    public Op transform(OpFuseJoin opFuseJoin, Op subOp) {
        if ( ! alwaysCopy && opFuseJoin.getSubOp() == subOp )
            return opFuseJoin ;
        return opFuseJoin.copy(subOp) ;
    }
    @Override
    public Op transform(OpSimilarityJoin opSimJoin, Op subOp) {
        if ( ! alwaysCopy && opSimJoin.getSubOp() == subOp )
            return opSimJoin ;
        return opSimJoin.copy(subOp) ;
    }
